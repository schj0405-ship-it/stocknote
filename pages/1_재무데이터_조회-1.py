...
# 색을 살짝 투명하게(rgba의 마지막 숫자가 투명도) 만들어서, 라이트 모드의
# 흰 배경 위에서는 옅은 색으로, 다크 모드의 어두운 배경 위에서는 은은한
# 톤으로 자동으로 어울리게 했습니다. (완전히 불투명한 색이면 다크 모드에서
# 화면과 안 어울리고 튀어 보일 수 있어서 이렇게 처리했습니다.)
METRIC_COLORS = {
    "매출액": "rgba(59, 130, 246, 0.20)",     # 파란 계열
    "영업이익": "rgba(34, 197, 94, 0.20)",    # 초록 계열
    "당기순이익": "rgba(249, 115, 22, 0.20)",  # 주황 계열
}
...
def style_growth_cell(value):
    ...
    감소(▼, 적자전환)는 빨강. 글자 색과 배경 둘 다 라이트/다크 모드 어느
    쪽에서 봐도 눈에 잘 띄도록 밝은 색(초록/빨강)과 반투명 배경을 같이 씁니다.
    """
    if isinstance(value, str):
        if value.startswith("▲") or value == "흑자전환":
            return "background-color: rgba(34, 197, 94, 0.25); color: #22c55e; font-weight: 700"
        if value.startswith("▼") or value == "적자전환":
            return "background-color: rgba(239, 68, 68, 0.25); color: #f87171; font-weight: 700"
    return ""
